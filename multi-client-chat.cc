...
private:
  ...
  virtual void StartApplication ()
  {
    Ptr<SocketFactory> socketFactory =
        GetNode()->GetObject<UdpSocketFactory>();
    ...
    m_socket->SetRecvCallback(
        MakeCallback(&ChatServerApp::HandleRead, this));
  }
...
private:
  ...
  void HandleResponse (Ptr<Socket> socket)
  {
    ...
    std::string data = std::string((char*)buf.data(),
                                   packet->GetSize());
...
int main (int argc, char *argv[])
{
  ...
  Ipv4InterfaceContainer i10 =
  address.Assign(d10);

  address.SetBase("10.2.1.0","255.255.255.0");

  Ipv4InterfaceContainer i20 =
  address.Assign(d20);

  /* SERVER */

  Ptr<ChatServerApp> server =
  CreateObject<ChatServerApp>();
  ...
  Ptr<ChatClientApp> clientA =
  CreateObject<ChatClientApp>();
